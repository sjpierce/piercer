...
test_that("Non-numeric p returns appropriate error", {
  expect_error(p2bfb(p = "a"), "p must be NA or numeric")
  expect_error(p2bfb(p = TRUE), "p must be NA or numeric")
  expect_error(p2bfb(p = factor("a")), "p must be NA or numeric")
})

test_that("Numeric p outside 0 to 1 returns appropriate error", {
  expect_error(p2bfb(p = -.1), "Numeric p must be a proportion between 0 and 1")
  expect_error(p2bfb(p = 1.1), "Numeric p must be a proportion between 0 and 1")
})
